            ...
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    ...
                    <div class="modal-body m-3">
                        <form id="create">
                            <div class="mb-3">
                                <label class="form-label required">Provinsi <i class="text-danger">*</i></label>
                                <select class="form-control form-control-sm choice provinsi" name="provinsi" id="provinsi">
...
<script>
    $(document).ready(function () {
        _table.set("{{ route('kab-kota.data') }}",
            [
                {data: 'nama', name: 'nama'},
                {data: 'provinsi_id.nama', name: 'provinsi_id.nama'},
        ...
        $(document).on('click','.create',function (e) {
            ...
            _ajax.post("{{ route('master-data.kab-kota.create') }}",data,
                (response)=>{
        ...
        $(document).on('click','.edit', function(e) {
            ...
            let id = $(this).data('id');
            _ajax.get(`{{ url('/auth/dashboard/master-kab-kota/edit') }}/${id}`,
                (response) => {
                    ...
                    if (response.status == 200) {
                        ...
                        let options = [];response.provinsi.forEach(e=>{response.data.provinsi_id==e.id?options.push(`<option value="${e.id}" selected>${e.nama}</option>`):options.push(`<option value="${e.id}">${e.nama}</option>`);});$('.select_edit').html(`<select class="form-control form-control-sm choice_edit edit_provinsi" name="edit_provinsi" id="provinsi">${options.join('')}</select>`);new Choices('.choice_edit');
                    }
                },
                (response) => {
        ...
        $(document).on('click','.update', function(e){
            ...
            let data = {
                ...
                'provinsi_id':$('.edit_provinsi option:selected').val(),
            }
            _input.loading.start(this);
            _ajax.put(`{{ url('/auth/dashboard/master-kab-kota/update') }}/${data.id}`,data,
                (response) => {
        ...
        $(document).on('click','.delete', function(e){
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    let id = $(this).data('id');
                    _ajax.get(`{{ url('/auth/dashboard/master-kab-kota/delete') }}/${id}`,
                        (response)=>{
                            if (response.status == 200) {
                                Swal.fire({
                                    title: 'Data terhapus!',
